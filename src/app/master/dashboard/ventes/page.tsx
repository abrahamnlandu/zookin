...
export default function CaissePage() {
  ...
  // Vérifier si la commande dépasse la limite
  const exceedsLimit = totals.totalVisitors > 50;
  ...
  // Traiter une vente normale (moins de 50 tickets)
  const processSale = () => {
    ...
    if (exceedsLimit) {
      alert('Cette commande dépasse la limite de 50 tickets. Veuillez la transférer au service financier.');
      return;
    }
  ...
  // Transférer une commande au service financier (plus de 50 tickets)
  const transferToFinance = () => {
    if (totals.totalVisitors === 0) {
      alert('Veuillez ajouter au moins un visiteur');
      return;
    }
    ...
    if (!exceedsLimit) {
      alert('Cette commande ne dépasse pas la limite de 50 tickets. Vous pouvez la traiter normalement.');
      return;
    }

    const transferToken = `TRANSFER-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`;
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .limit-warning { background-color: #fff3cd; border: 1px solid #ffeaa7; padding: 10px; border-radius: 5px; margin: 10px 0; }
          </style>
        </head>
        <body>
          <div class="header">
      ...
      {exceedsLimit && (
        ...
        >
          <div className="flex items-center">
            <FiAlertTriangle className="text-yellow-600 dark:text-yellow-400 mr-3 text-xl" />
            <div>
              <h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-300">
                Commande groupée détectée
              </h3>
              <p className="text-sm text-yellow-700 dark:text-yellow-400 mt-1">
                Cette commande de <strong>{totals.totalVisitors} tickets</strong> dépasse la limite de 50 tickets. 
          ...
          <div className="mt-4 flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
            ...
            <div className="flex items-center">
              <span className={`text-lg font-bold ${
                totals.totalVisitors > 50 ? 'text-red-600' : 'text-green-600'
              }`}>
                {totals.totalVisitors}
              </span>
              <span className="text-sm text-gray-500 ml-1">/ 50 max</span>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
              Commandes de plus de 50 tickets
            </p>
          </motion.div>
